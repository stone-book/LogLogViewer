...
namespace Hnx8.ReadJEnc
{
    ...
    {   ////////////////////////////////////////////////////////////////////////
        ...
        private const int maxTempByteLen = 65536;
        ...
        public FileReader(FileInfo file)
            : this((file.Length <= 65536 ? (int)file.Length : maxTempByteLen)) { }
                ...
                {   //オープン後の実際のファイルサイズによるチェック
                    ...
                    {   //一定サイズ以上の大きいファイルなら、BOM/マジックナンバー判定に必要な先頭バイトを読み込み、判断
                        ...
                        {   //残りの読みこみ（ただし非テキストと確定した場合は省略）
                            this.Length += stream.Read(Bytes, this.Length, maxTempByteLen - this.Length);
                        }
                    }
                    else
